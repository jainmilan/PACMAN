...
def run_3(df):
    feature_matrix = df.reset_index()[["ext_temperature", "status"]]
    Y = df.reset_index().int_temperature.diff()

    n = len(df)

    regr = linear_model.LinearRegression()
    regr.fit(feature_matrix.head(n-1), Y.tail(n-1))

    model_dir = '../Results/Prediction/models/lr/'
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    joblib.dump(regr, model_dir + 'regr.pkl')
...
def run_classifier_model_1(df, usage):
    ...
    joblib.dump(classify, model_dir + 'lr.pkl')
    return True
...
def run_regr_model_1(df):
    ...
    joblib.dump(regr, model_dir + 'lr_1.pkl')
    return True
...
def run_regr_model_2(df):
    ...
    joblib.dump(regr, model_dir + 'lr_2.pkl')
    return True