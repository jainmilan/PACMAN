...
def run_classifier(df, usage):
    data_size = len(df)

    feature_matrix_classify = pandas.DataFrame()
    feature_matrix_classify["int_temperature"] = df.tail(data_size).reset_index()["int_temperature"]
    feature_matrix_classify["status"] = df.head(data_size).reset_index()["status"]

    Y_classify = df.tail(data_size).reset_index().status

    classify = DecisionTreeClassifier(random_state=0)
    classify.fit(feature_matrix_classify, Y_classify)

    model_dir = '../Results/Prediction/models/dt/'
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    joblib.dump(classify, model_dir + 'clf.pkl')
...
def run_classifier_model_1(df, usage):
    ...
    joblib.dump(classify, model_dir + 'dt.pkl')
    return True
...
def run_regr_model_1(df):
    ...
    joblib.dump(regr, model_dir + 'dt_1.pkl')
    return True
...
def run_regr_model_2(df):
    ...
    joblib.dump(regr, model_dir + 'dt_2.pkl')
    return True
